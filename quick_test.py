...
def quick_test(script_file: str, output_dir: str = "outputs/quick_test"):
    ...
    config = build_generation_config(device="cuda")
    ...
    for i, panel in enumerate(board.panels):
        ...
        if getattr(board, "render_plan", None) and i < len(board.render_plan):
            eval_prompt = board.render_plan[i].get("compiled_prompt") or panel.enhanced_prompt or panel.raw_prompt
        else:
...
if __name__ == "__main__":
    # 默认测试文件
    script_file = sys.argv[1] if len(sys.argv) > 1 else "storygen/data/TaskA/06.txt"
