...
class NarrativeGenerationPipeline:
    ...
    def base_pipe(self):
        ...
        if self._base_pipe is None:
            ...
            from src.utils.mirror_config import verify_model_integrity, get_models_cache_dir

            model_name = self.config.get("base_model", "adamo1139/stable-diffusion-3.5-medium-ungated")
            cache_dir = get_models_cache_dir()
            print(f"[Pipeline] Loading SD3.5 Base Model: {model_name}")
            ...
            self._base_pipe = StableDiffusion3Pipeline.from_pretrained(
                model_name,
                **load_kwargs
            ).to(self.device)
