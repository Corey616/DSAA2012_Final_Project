...
"""
    ...
    def call_llm_for_analysis(self, parsed_script: Dict) -> str:
        ...
        if self.client is None:
            raise RuntimeError(
                f"[Parser] No LLM client available for backend '{self.llm_backend}'. "
                "LLM-only parsing is enforced. Install required package (openai, anthropic) "
                "or ensure Qwen model is cached and GPU is available."
            )
        ...
        if self.llm_backend in ["local", "api_openai"]:
            try:
                ...
                raise RuntimeError(
                    f"[Parser] LLM call failed ({self.model_name}): {e}. "
    ...
    def parse_llm_response(self, llm_output: str, raw_text: str = "", scenes: List = None) -> ProductionBoard:
        ...
        try:
            ...
            raise ValueError(
                f"[Parser] Failed to parse LLM JSON output: {e}. "
                f"Raw output (first 300 chars): {llm_output[:300]}"
            ) from e
        ...
        # LLM-ONLY: Verify all characters have complete descriptions from LLM
        # No random generation — LLM must provide all visual details
        for char_name, char_info in characters.items():
            if not char_info.visual_description or "person" in char_info.visual_description.lower()[:30]:
                raise ValueError(
                    f"[Parser] Character '{char_name}' has incomplete visual_description: "
                    f"'{char_info.visual_description or '(empty)'}'. "
                    "LLM-only parsing enforced — the LLM must provide specific visual descriptions. "
                    "Check the SYSTEM_PROMPT for visual description requirements."
                )
