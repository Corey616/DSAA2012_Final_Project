...
def match_histogram(source: Image.Image, reference: Image.Image, blend: float = 1.0) -> Image.Image:
    """
    Match color histogram of source image to reference image.
    Uses per-channel histogram matching to prevent progressive desaturation
    across a sequence of generated frames.

    Args:
        source: Image to adjust (PIL Image)
        reference: Reference image whose color distribution to match (PIL Image)
        blend: Blending factor (0.0 = no change, 1.0 = full match to reference)
    ...
    for c in range(3):
        ...
        # Compute histograms
        src_hist, _ = np.histogram(src_ch, bins=256, range=(0, 255), density=True)
        ref_hist, _ = np.histogram(ref_ch, bins=256, range=(0, 255), density=True)
        ...
        # Create pixel value mapping via inverse CDF interpolation
        mapping = np.interp(src_cdf, ref_cdf, np.arange(256))

        # Apply mapping to source channel
        matched_ch = np.interp(src_ch.flatten(), np.arange(256), mapping)
        matched[:, :, c] = matched_ch.reshape(src_ch.shape)

    # Blend between original and fully-matched for stability
    result = (1.0 - blend) * src + blend * matched
