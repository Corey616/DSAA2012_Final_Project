...
# Now lives inside storygen/models/ for clean packaging
STORYGEN_DIR = Path(__file__).parent.parent  # storygen/
MODELS_CACHE_DIR = STORYGEN_DIR / "models"  # storygen/models/
...
def get_sdxl_model_path() -> str:
    """
    Get the local SDXL model path.
    Uses pre-cached model at /data/lizhenzhuo/models/stable-diffusion-xl-base-1.0
    Falls back to HF model ID if not available.
    """
...
def get_qwen_model_path() -> str:
    """Get the local Qwen2.5-7B model path or fallback to HF ID.
    Checks shared cache and project cache."""
    for path in [
        Path("/data/shared_data/FuLi/data_cache/Qwen2.5-7B-Instruct"),
        Path("/data/lizhenzhuo/models/Qwen2.5-7B-Instruct"),
    ]:
...
def setup_china_mirrors():
    ...
    3. Auto-detect proxy: if http_proxy is set, use direct download (not hf-transfer)
    """
    # Use original HuggingFace - most stable
    os.environ.pop("HF_ENDPOINT", None)  # Remove mirror setting
    
    # Check if proxy is active (proxy is more reliable than hf-transfer)
    if os.environ.get("http_proxy") or os.environ.get("https_proxy"):
        os.environ.pop("HF_HUB_ENABLE_HF_TRANSFER", None)  # Disable hf-transfer when proxy is on
        print("[Mirror] HuggingFace Hub configured (via proxy)")
        print(f"   • Proxy: {os.environ.get('http_proxy', 'N/A')}")
    else:
        # Enable hf-transfer for faster direct downloads
...
def verify_model_integrity(
    ...
    # Support local path (not HF cache format)
