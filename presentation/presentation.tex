...
% 7. Limitations
\section{LIMITATIONS}
\begin{frame}{Case 10: Baby - Non-Human Consistency Challenge}
    \begin{columns}
        \column{0.65\textwidth}
        \includegraphics[width=\textwidth,height=0.75\textheight,keepaspectratio]{images/case10_storyboard.png}
        \column{0.35\textwidth}
        \footnotesize
        \textbf{Metrics:}
        \begin{itemize}
            \item CLIP: 0.294, Consistency: 0.367
            \item Overall: 0.323
        \end{itemize}
        \vspace{0.3em}
        \textbf{Challenges:}
...
\begin{frame}{Summary of Limitations}
    \begin{columns}
        ...
        \textbf{Challenges:}
        \begin{itemize}
            \item \textbf{Non-human characters}: Animals, babies inconsistent
            \item \textbf{Abstract entities}: Robots generated as humans
            \item \textbf{Clothing details}: Minor variations
        \end{itemize}
        \column{0.5\textwidth}
        \footnotesize
        \textbf{Strengths:}
...
\begin{frame}{Future Work}
    \begin{columns}
        ...
        \begin{itemize}
            \item Integrate IP-Adapter or ControlNet for stronger character anchoring
            \item Implement cross-frame attention mechanisms (ReDiStory, ICSA)
            \item Use face embedding-based consistency loss
        \end{itemize}
        \column{0.5\textwidth}
        \footnotesize
        \textbf{Non-Human Characters:}
        \begin{itemize}
            \item Specialized prompts for abstract entities
            \item Fine-tune SDXL on non-human characters
            \item Use reference images as additional conditioning
        \end{itemize}
    \end{columns}
    \vspace{1em}
    \begin{block}{Long-term Vision}
        \footnotesize
        Extend to video generation with temporal consistency; integrate TTS for narrated story output
    \end{block}
\end{frame}
